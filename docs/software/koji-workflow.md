...
-   Clone the osg-build GitHub repo: &lt;pre&gt;<span class="twiki-macro UCL_PROMPT"></span> git clone <https://github.com/opensciencegrid/osg-build>&lt;/pre&gt;
-   Add this directory to your `$PATH`
-   Run &lt;pre&gt;<span class="twiki-macro UCL_PROMPT"></span> osg-koji setup&lt;/pre&gt; to set up the koji configuration and certificates in `~/.osg-koji`
...
-   Run &lt;pre&gt;<span class="twiki-macro UCL_PROMPT"></span> osg-koji setup&lt;/pre&gt; to set up the appropriate configuration and certificates in `~/.osg-koji`
...
``` screen
%UCL_PROMPT% osg-build koji %RED%package_name%ENDCOLOR% 
```
...
When you do a non-scratch build, it will build with the *osg-el6* and *osg-el7* targets. This will assign your build the *osg-3.3-el6-development* and *osg-3.3-el7-development* tags (and your package will be assigned the *osg-el6* and *osg-el7* tags). If successful, your build will end up in the Koji *osg-minefield* yum repos and will eventually show up in the *osg-development* yum repos. This is a high latency process.
...
<http://koji.chtc.wisc.edu/koji/index>
...
<http://koji.chtc.wisc.edu/koji/packageinfo?packageID=56>

And the lcmaps-1.6.6-1.1.osg33.el6 build is here:

<http://koji.chtc.wisc.edu/koji/buildinfo?buildID=7427>
...
``` screen
yum localinstall %RED%package%ENDCOLOR%.rpm
```
...
-   The koji-hub repository can be created manually by running &lt;pre class="screen"&gt;osg-koji regen-repo %RED%tag\_name%ENDCOLOR%&lt;/pre&gt; For example, the tag name for osg-development in 3.3 on el6 is "osg-3.3-el6-development". Likely, you won't need to do this when kojira is working.
...
``` screen
%UCL_PROMPT% osg-promote -r %RED%OSGVER%ENDCOLOR%-testing %RED%package_name%ENDCOLOR% 
```

If you want to promote a specific version:

``` screen
%UCL_PROMPT% osg-promote -r %RED%OSGVER%ENDCOLOR%-testing %RED%build_name%ENDCOLOR% 
```

Where OSGVER is the OSG major version that you are promoting for (e.g. 3.3).
...
`package_name` is the bare package name without version, e.g. `gratia-probe`. `build_name` is a full `name-version-revision.disttag` such as `gratia-probe-1.17.0-2.osg33.el6`. For `osg-promote`, you may omit the `.osg33.el6` or =.osg33.el7=; the script will add the appropriate disttag on.
